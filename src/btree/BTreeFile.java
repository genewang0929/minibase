...
public class BTreeFile extends IndexFile
	...
	throws PinPageException {
		if (DEBUG) {
			System.out.println("[BTreeFile] pin page: " + pageno.pid);
		}
		try {
			Page page = new Page();
	...
	throws UnpinPageException {
		if (DEBUG) {
			System.out.println("[BTreeFile] unpin page: " + pageno.pid);
		}
		try {
...

	{
		...
		// IMPORTANT: leaf page insert logic
			...
			// IMPORTANT: move current leaf page records to new leaf page
					...
					if ( (4 + leafPage.available_space()) <= ((MAX_SPACE - HFPage.DPFIXED) / 2) ) {
						...
						if (siblingPage.redistribute( leafPage, parentPage, direction, deletedKey)) {
							...
							return null;
						            ...
						            ( (MAX_SPACE - HFPage.DPFIXED)  - leafPage.available_space())) {
