...
public class Sort extends Iterator implements GlobalConst
{
  ...
  private Vector100Dtype Target;  // Target vector for k-NN sorting
  private int k;                  // Number of nearest neighbors to retain
	      ...
	      ) throws IOException, SortException
  {
    ...
    if (_in[sort_fld - 1].attrType == AttrType.attrVector100D) {
	Q = new pnodeSplayPQ(_sort_fld, _in[_sort_fld - 1], order) {
	  @Override
	  public int pnodeCMP(pnode p1, pnode p2) {
	    try {
              // Use the provided CompareTupleWithTuple method.
              return TupleUtils.CompareTupleWithTuple(_in[_sort_fld - 1],
                                                        p1.tuple, _sort_fld,
                                                        p2.tuple, _sort_fld);
            } catch (Exception e) {
              e.printStackTrace();
	   ...
	   Exception
  {
    ...
    if (k > 0) {
	return (topK != null && !topK.isEmpty()) ? topK.poll() : null;
    }
  ...
  private PriorityQueue<Tuple> topK; // Priority queue for k-NN
  private boolean kSorted = false;   // Flag to track k-NN computation

  private void selectTopK() throws IOException, UnknowAttrType, SortException, Exception {
    if (kSorted) return; // Prevent recomputation

    // Max-Heap to keep k nearest tuples
    topK = new PriorityQueue<>(k, (t1, t2) -> {
      try {
        return TupleUtils.CompareTupleWithTuple(_in[_sort_fld - 1], t2, _sort_fld, t1, _sort_fld);
      } catch (Exception e) {
        e.printStackTrace();
        return 0;
      }
    ...
    while ((t = delete_min()) != null) {
      if (topK.size() < k) {
        topK.add(t);
      } else if (TupleUtils.CompareTupleWithTuple(_in[_sort_fld - 1], t, _sort_fld, topK.peek(), _sort_fld) < 0) {
        topK.poll(); // Remove the farthest tuple
        topK.add(t);
      }
    }
